...
- 계산 코드: `dashboards/stock-outlook/model.js`. 검증: `node _src/test_stock_fundamental.cjs`.
- 기존 가격 모델은 [통계 참고 화면](https://psr8989.github.io/Github/dashboards/stock-outlook/price-only.html)에 보존했습니다. 아래 v1 검증 수치는 v2 정확도가 아닙니다.


[대시보드 열기](https://psr8989.github.io/Github/dashboards/stock-outlook/)

현대차·SK하이닉스·삼성전자의 2026-09-11 종가 기준, 2026년 12월까지의 통계 시나리오입니다. 실제 주가/예상 경로, 원화/기준가 100 비교, 월말 가격 테이블, CSV 다운로드와 과거 6구간 검증 결과를 제공합니다.

- 데이터: Yahoo Finance 일별 종가 및 조정 종가. 실시간 자동 갱신 없음.
- 모델: 126거래일 로그 수익률의 연환산 평균을 25% 반영(±35% 제한), 표본 변동성 기반 로그정규 P10/P50/P90.
- 재생성: `python _src/build_stock_outlook.py` (표준 라이브러리만 사용, 인터넷 필요). 분석 기준일은 스크립트의 `ASOF`로 고정되어 있습니다.
- 출력: `dashboards/stock-outlook/data.json`, `data.js`. 조회 시 외부 차트 라이브러리나 API가 필요하지 않습니다.
- 검증: 21거래일 예측 6회, 각 시점 이전 데이터만 사용. MAPE·가격 유지 기준·범위 적중 횟수 공개. 1개월 검증은 연말 정확도를 보장하지 않습니다.
- 제한: 미래 실적·환율·금리·배당을 직접 예측하지 않으며, 목표주가나 투자 권고가 아닙니다.
